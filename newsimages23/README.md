...
Our work is motivated by the challenges posed by the [NewsImages: Connecting Text and Images task](https://multimediaeval.github.io/editions/2023/tasks/newsimages) in the MediaEval 2023 competition.
This competition focuses on understanding the relationship between textual and visual content in news articles. Participants are tasked with developing models to describe and predict the relationships between images and textual elements (headlines and snippets) in news articles.
...
During the testing phase, participants received a new set of news headlines without images, along with a pool of shuffled images.
...
Given the relatively small size of our dataset compared to the CLIP model's training dataset, and our experiments showing that fine-tuning the CLIP model does not improve its original performance, we focused on dataset tuning by rephrasing the data.
The goal is to improve the compatibility of our data with the CLIP model, thereby achieving better performance in matching relevant images to rephrased headlines.
...
1. Prepare the Dataset: Ensure that you have the dataset ready and place it in the appropriate directory.
Ensure you have extracted the contents of the .zip file.
...
Leverage CLIP's image embeddings to convert all images in the pool to embeddings.
...
### Calculate the average hit@K score

Average the Hits@K scores across all images by summing the Hits@K scores for all images and dividing the sum by the total number of images.
The average Hits@K score ranges between 0 and 1.
