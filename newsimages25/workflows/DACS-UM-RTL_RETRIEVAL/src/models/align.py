...
@dataclass
class AlignWrapper(VLMWrapperRetrieval):
    model: Any = field(
        default_factory=lambda:AlignModel.from_pretrained("kakaobrain/align-base",device_map={"": 0}, torch_dtype=torch.float16))
    
    processor: Any = field(default_factory=lambda: AlignProcessor.from_pretrained("kakaobrain/align-base"))
