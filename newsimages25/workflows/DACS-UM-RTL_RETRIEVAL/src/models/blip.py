...
@dataclass
class BlipWrapper(VLMWrapperRetrieval):
    model: Any = field(
        default_factory=lambda: BlipForImageTextRetrieval.from_pretrained(
            "Salesforce/blip-image-captioning-base", device_map={"": 0}, torch_dtype=torch.float16
        )
    )
    processor: Any = field(default_factory=lambda: AutoProcessor.from_pretrained("Salesforce/blip-image-captioning-base"))
